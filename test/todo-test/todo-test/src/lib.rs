...
wit_bindgen::generate!({
    path: "../target/wit",
    world: "todo-test-template-dot-os-v0",
...
fn handle_message(our: &Address) {
    ...
    print_to_terminal(0, "todo_test: start");
    ...
    increment_counter(&our_todo_address, 1);

    let local_messages = get_messages(&our_todo_address);
    if !local_messages
        .iter()
        .any(|m| m == "Counter incremented by 1")
    {
        fail_with(format!(
            "local todo did not store expected message: {local_messages:?}"
        ));
    }

    let status = get_status(&our_todo_address);
    let counter = status.counter;
    if counter != 1 {
        fail_with(format!(
            "local todo did not increment correctly: {counter:?}"
        ));
    }
...
fn fail_with(message: impl Into<String>) -> ! {
    ...
    let log = format!("todo_test: error: {message}");
